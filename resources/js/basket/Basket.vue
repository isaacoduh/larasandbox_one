  ...
  <div>
    ...
    <div class="row" v-else>
      ...
      <div class="col-md-4">
        ...
        <transition-group>
        <div v-for="item in basket" :key="item.orderable.id">
            <div class="pt-2 pb-2 border-top d-flex justify-content-between">
                <span>
                <router-link
                    :to="{name: 'orderable', params: {id: item.orderable.id}}"
                >{{ item.orderable.title }}</router-link>
                </span>
            ...
            <div class="pt-2 pb-2 text-right">
                ...
                @click="$store.dispatch('removeFromBasket', item.orderable.id)"
                >
...
export default {
  ...
  computed: {
    ...
    success(){
        return !this.loading && 0 === this.itemsInBasket && this.orderAttempted;
    }
  },
  methods: {
      async order(){
          ...
          try {
              await axios.post(`/api/checkout`, {
                  ...
                  orders: this.basket.map(basketItem => ({
                      orderable_id: basketItem.orderable.id,
